    ...
    async start(controller) {
      ...
      try {
        for (;;) {
          ...
          if (!page.fromCache) {
            // После свежей загрузки WB остывает ~10+ минут (проверено: через 75 с
            // стабильно 429, а лишние попытки продлевают блокировку). В один прогон
            // это не влезает, поэтому останавливаемся: страница уже в кэше,
            // следующий запуск начнёт сразу со следующей.
            send({
              log:
                `страница сохранена; следующую (rrdid=${rrdid}) возьмёт следующий запуск — ` +
                "WB нужно ~10 мин остыть после тяжёлой страницы",
            });
            break;
          }
        }
