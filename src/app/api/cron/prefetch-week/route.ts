    ...
    async start(controller) {
      ...
      try {
        for (;;) {
          ...
          if (!page.fromCache) {
            // Страницы по 20k WB отдаёт стабильно; между ними достаточно
            // паузы под лимит «1 запрос/мин». Если следующая страница + пауза
            // не влезают в бюджет — остановимся, остаток доберёт следующий запуск.
            if (Date.now() - started + WB_WAIT_MS + 60_000 > TIME_BUDGET_MS) {
              send({ log: `страница сохранена; следующую (rrdid=${rrdid}) возьмёт следующий запуск (бюджет времени)` });
              break;
            }
            send({ log: `пауза ${WB_WAIT_MS / 1000} сек (лимит WB), затем следующая страница` });
